                ...
                <div class="bg-gray-100 dark:bg-gray-700 px-4 py-3 flex justify-between items-center border-b border-gray-200 dark:border-gray-600">
                    <div>
                        ...
                        <h3 class="font-bold text-lg text-gray-800 dark:text-white inline-block">
                            <?php echo e($principle->code ? $principle->code . ' - ' : ''); ?><?php echo e($principle->name); ?>

                        </h3>
                    </div>
                    <div class="flex space-x-2">
                        <button onclick="openModal('edit', <?php echo e($principle); ?>)" class="text-xs text-blue-600 hover:text-blue-800 font-semibold">Edit</button>
                        <button onclick="confirmDelete(<?php echo e($principle->id); ?>)" class="text-xs text-red-600 hover:text-red-800 font-semibold">Delete</button>
                        <button onclick="openModal('create', <?php echo e($principle->id); ?>, 'criteria')" class="text-xs bg-indigo-600 text-white px-2 py-1 rounded hover:bg-indigo-700">+ Criteria</button>
                    </div>
                </div>

                <!-- Children (Criteria) -->
                <div class="bg-white dark:bg-gray-800 p-4 space-y-4">
                    <?php $__currentLoopData = $principle->children; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $criteria): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            ...
                            <div class="flex justify-between items-start mb-2">
                                <div>
                                    ...
                                    <span class="font-medium text-gray-800 dark:text-gray-200">
                                        <?php echo e($criteria->code ? $criteria->code . ' - ' : ''); ?><?php echo e($criteria->name); ?>

                                    </span>
                                </div>
                                <div class="flex space-x-2">
                                    <button onclick="openModal('edit', <?php echo e($criteria); ?>)" class="text-xs text-blue-500 hover:text-blue-700">Edit</button>
                                    <button onclick="confirmDelete(<?php echo e($criteria->id); ?>)" class="text-xs text-red-500 hover:text-red-700">Delete</button>
                                    <button onclick="openModal('create', <?php echo e($criteria->id); ?>, 'indicator')" class="text-xs bg-green-50 text-green-700 border border-green-200 px-2 py-0.5 rounded hover:bg-green-100">+ Indicator</button>
                                </div>
                            </div>

                            <!-- Children (Indicator) -->
                            <div class="ml-4 space-y-3 mt-2">
                                <?php $__currentLoopData = $criteria->children; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $indicator): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        ...
                                        <div class="flex justify-between items-start mb-2">
                                            <div>
                                                ...
                                                <span class="text-sm font-semibold text-gray-700 dark:text-gray-300">
                                                    <?php echo e($indicator->code ? $indicator->code . ' - ' : ''); ?><?php echo e($indicator->name); ?>

                                                </span>
                                            </div>
                                            <div class="flex space-x-2">
                                                <button onclick="openModal('edit', <?php echo e($indicator); ?>)" class="text-xs text-blue-400 hover:text-blue-600">Edit</button>
                                                <button onclick="confirmDelete(<?php echo e($indicator->id); ?>)" class="text-xs text-red-400 hover:text-red-600">Delete</button>
                                                <button onclick="openModal('create', <?php echo e($indicator->id); ?>, 'parameter')" class="text-xs bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 border border-gray-300 dark:border-gray-600 px-1.5 py-0.5 rounded hover:bg-gray-50 dark:hover:bg-gray-700">+ Param</button>
                                                <button onclick="openModal('create', <?php echo e($indicator->id); ?>, 'verifier')" class="text-xs bg-white dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-800/50 px-1.5 py-0.5 rounded hover:bg-indigo-50 dark:hover:bg-indigo-900/50">+ Verifier</button>
                                            </div>
                                        </div>

                                        <!-- Children (Parameter/Verifier) -->
                                        <div class="ml-2 pl-2 border-l border-gray-300 dark:border-gray-600 space-y-2">
                                            <?php $__currentLoopData = $indicator->children; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $child): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                <!-- Parameter/Verifier Item -->
                                                <div class="flex justify-between items-start text-xs group">
                                                    <div class="flex-1 pr-2">
                                                        <?php if($child->type === 'verifier'): ?>
                                                        ...
                                                        <?php endif; ?>
                                                        <span class="text-gray-700 dark:text-gray-300"><?php echo e($child->name); ?></span>
                                                    </div>
                                                    <div class="flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                        <button onclick="openModal('edit', <?php echo e($child); ?>)" class="text-blue-400 hover:text-blue-600">Edit</button>
                                                        <button onclick="confirmDelete(<?php echo e($child->id); ?>)" class="text-red-400 hover:text-red-600">Del</button>
                                                        <?php if($child->type === 'parameter'): ?>
                                                            <button onclick="openModal('create', <?php echo e($child->id); ?>, 'verifier')" class="text-indigo-500 hover:text-indigo-700">+ Verifier</button>
                                                        <?php endif; ?>
                                                    </div>
                                                </div>
                                                ...
                                                <?php if($child->children->count() > 0): ?>
                                                    <div class="ml-2 mt-1 space-y-1">
                                                        <?php $__currentLoopData = $child->children; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $verifier): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                            <div class="flex justify-between items-center text-xs pl-2 border-l border-indigo-200 group">
                                                                <span class="text-gray-600 dark:text-gray-400 flex-1">
                                                                    <span class="text-indigo-500">[V]</span> <?php echo e($verifier->name); ?>

                                                                </span>
                                                                <div class="flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                                    <button onclick="openModal('edit', <?php echo e($verifier); ?>)" class="text-blue-400 hover:text-blue-600">Edit</button>
                                                                    <button onclick="confirmDelete(<?php echo e($verifier->id); ?>)" class="text-red-400 hover:text-red-600">Del</button>
                                                                </div>
                                                            </div>
...
<script>
    function openModal(mode, data = null, childType = null) {
        ...
        if (mode === 'create') {
            ...
            form.action = "<?php echo e(route('ispo.admin.items.store')); ?>";
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\plantation.oilpam.my.id\Modules/SystemISPO\resources/views/ispo/admin/items/index.blade.php ENDPATH**/ ?>