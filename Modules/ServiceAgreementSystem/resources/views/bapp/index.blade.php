<x-serviceagreementsystem::layouts.master :title="'Manajemen BAPP'">
    @push('styles')
    <style>
        .bapp-table-wrapper {
            background: white;
    ...
    <div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        ...
        <div class="bapp-table-wrapper">
            <div class="bapp-header">
                ...
                <a href="{{ route('sas.bapp.create') }}" class="btn-premium">
            ...
            <table class="bapp-table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td style="font-weight: 700; color: #1e293b;">{{ $bapp->bapp_number }}</td>
                        <td style="color: #64748b;">{{ $bapp->bapp_date->format('d M Y') }}</td>
                        <td>
                            @if($bapp->document_link)
                            <a href="{{ $bapp->document_link }}" target="_blank" class="badge-premium" style="background: #fef2f2; color: #ef4444; border: 1px solid #fca5a5;">
                        ...
                        <td>
                            <div style="font-weight: 600; color: #334155;">{{ $bapp->contractor->name ?? 'Multi Kontraktor' }}</div>
                        </td>
                        <td style="color: #475569;">{{ $bapp->job->name ?? '-' }}</td>
                        <td>
                            <span class="badge-premium">{{ $bapp->submissions_count }} SPK/USPK</span>
                        </td>
                        <td>
                            <a href="{{ route('sas.bapp.show', $bapp->id) }}" class="btn-outline">
