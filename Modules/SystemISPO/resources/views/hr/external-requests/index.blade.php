...
<div class="payflow-container">
    ...
    <div class="pf-page-header">
        ...
        <a href="{{ route('hr.external-requests.create') }}" class="pf-btn pf-btn-dark">
    ...
    {{-- Filter Bar --}}
    <form method="GET" action="{{ route('hr.external-requests.index') }}" class="pf-filter-bar shadow-sm">
        <div class="pf-search-wrapper">
            <i class="fas fa-search pf-search-icon"></i>
            <input type="text" name="q" value="{{ request('q') }}" placeholder="Cari nomor referensi atau pihak peminta..." class="pf-search-input">
        </div>
        ...
        <div style="display: flex; align-items: center; border-left: 1px solid #e5e7eb; padding-left: 16px; gap: 12px;">
            ...
            <a href="{{ route('hr.external-requests.index') }}" class="pf-btn-icon pf-btn flex items-center justify-center" title="Reset"><i class="fas fa-sync-alt text-xs"></i></a>
        </div>
    </form>
                    ...
                    @forelse($tickets as $ticket)
                        <tr>
                            <td>
                                <div style="display: flex; align-items: center; gap: 12px;">
                                    ...
                                    <div>
                                        ...
                                        <div style="font-size: 12px; color: #4b5563; font-weight: 500; margin-top: 2px;">{{ $ticket->judul_permintaan ?: 'Tanpa Judul' }}</div>
                                    </div>
                                </div>
                            </td>
                            <td>
                                <span style="color: #6b7280; font-weight: 500;">{{ $ticket->kategori_data ? ucwords(str_replace('_', ' ', $ticket->kategori_data)) : '-' }}</span>
                            </td>
                            <td>
                                <span class="pf-status-pill dot-{{ $ticket->status_proses }}">{{ $ticket->status_proses }}</span>
                            </td>
                            <td>
                                @php $isLate = optional($ticket->deadline)->isPast() && $ticket->status_proses !== 'selesai'; @endphp
                                <div style="display: flex; flex-direction: column;">
                                    <span style="font-weight: 600; color: {{ $isLate ? '#ef4444' : '#4b5563' }};">
                            ...
                            <td style="text-align: right;">
                                <a href="{{ route('hr.external-requests.show', $ticket) }}" class="pf-btn pf-btn-icon">
