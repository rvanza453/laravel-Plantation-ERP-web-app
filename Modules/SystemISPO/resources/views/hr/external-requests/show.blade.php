...
<div class="pf-show-wrapper">
    ...
    @if(session('share_link'))
        <div style="background: #232220; color: #fff; padding: 28px; border-radius: 28px; margin-bottom: 32px; border: 1px solid #232220;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
                ...
                <button onclick="navigator.clipboard.writeText('{{ session('share_link') }}'); alert('Link disalin!')" 
    ...
    <div class="pf-header">
        <div>
            <div class="pf-meta">
                <span style="color: #6b7280;">REF #{{ $ticket->nomor_referensi }}</span> 
                <span style="color: #d1d5db;">•</span>
                <span class="pf-status-pill dot-{{ $ticket->status_proses }}" style="padding: 2px 10px; font-size: 11px; border:none; background: #f3f4f6;">{{ $ticket->status_proses }}</span>
            </div>
            <h1 class="pf-title">{{ $ticket->judul_permintaan ?: 'Tiket Permintaan Data' }}</h1>
            <div style="font-size: 14px; font-weight: 500; color: #6b7280; display: flex; align-items: center; gap: 8px;">
                ...
                <span>{{ ucwords(str_replace('_', ' ', $ticket->pihak_peminta)) }}</span>
            </div>
        </div>
        <div style="display: flex; gap: 12px;">
            ...
            <a href="{{ route('hr.external-requests.edit', $ticket) }}" class="pf-btn pf-btn-dark">
                Edit Tiket
            </a>
        </div>
    </div>
    ...
    <div class="pf-main-grid">
        ...
        <div style="display: flex; flex-direction: column; gap: 20px;">
            ...
            <div class="pf-card-light">
                ...
                <div class="pf-info-grid">
                    <div>
                        ...
                        <div class="pf-info-value">{{ optional($ticket->tanggal_surat_masuk)->format('d F Y') ?: '-' }}</div>
                    </div>
                    <div>
                        <div class="pf-info-label">Judul Permintaan</div>
                        <div class="pf-info-value">{{ $ticket->judul_permintaan ?: '-' }}</div>
                    </div>
                    <div>
                        <div class="pf-info-label">Kategori Data</div>
                        <div class="pf-info-value" style="text-transform: capitalize;">{{ $ticket->kategori_data ? str_replace('_', ' ', $ticket->kategori_data) : '-' }}</div>
                    </div>
                    <div>
                        <div class="pf-info-label">PIC Internal</div>
                        <div class="pf-info-value">{{ $ticket->picUser?->name ?? '-' }}</div>
                    </div>
                </div>

                <div class="pf-info-label">Deskripsi Permintaan</div>
                <div class="pf-desc-box">
                    {{ $ticket->deskripsi_permintaan ?: '-' }}
                </div>

                @if($ticket->catatan_internal)
            ...
            <div class="pf-card-light">
                ...
                <div class="text-muted">Berkas yang diunggah untuk tiket ini ({{ $ticket->attachments->count() }})</div>
                    ...
                    @forelse($ticket->attachments as $file)
                        <div class="pf-attachment-card">
                            <div style="display: flex; align-items: center; gap: 16px;">
                                ...
                                <div>
                                    <div style="font-size: 14px; font-weight: 500; color: #111;">{{ $file->file_name }}</div>
                                    <div style="font-size: 12px; color: #9ca3af;">{{ ucwords(str_replace('_', ' ', $file->kategori_lampiran)) }} • {{ number_format(($file->file_size ?? 0)/1024, 1) }} KB</div>
                                </div>
                            </div>
                            <div style="display: flex; gap: 8px;">
                                <a href="{{ route('hr.external-requests.attachments.preview', [$ticket, $file]) }}" target="_blank" class="pf-btn-icon pf-btn"><i class="fas fa-eye text-xs"></i></a>
                                <a href="{{ route('hr.external-requests.attachments.download', [$ticket, $file]) }}" class="pf-btn-icon pf-btn"><i class="fas fa-download text-xs"></i></a>
                            </div>
                        </div>
                    @empty
            ...
            <div class="pf-card-light">
                ...
                <div class="pf-table-wrapper">
                    <table class="pf-table">
                        ...
                        <tbody>
                            @forelse($ticket->shareTokens as $token)
                                <tr style="{{ $token->revoked_at ? 'opacity: 0.4;' : '' }}">
                                    <td style="font-family: monospace; font-size: 14px; color: #111;">{{ $token->token_hint }}...</td>
                                    <td>
                                        <div style="display: flex; gap: 4px;">
                                            @if($token->allow_download) <span style="font-size: 10px; font-weight: 600; padding: 2px 8px; background: #f3f4f6; color: #111; border-radius: 100px;">Download</span> @endif
                                            @if($token->allow_preview_only) <span style="font-size: 10px; font-weight: 600; padding: 2px 8px; background: #f3f4f6; color: #111; border-radius: 100px;">Preview Only</span> @endif
                                        </div>
                                    </td>
                                    <td style="color: #6b7280;">{{ $token->view_count }} / {{ $token->max_views ?: '∞' }}</td>
                                    <td style="color: #6b7280; font-size: 13px;">{{ optional($token->expires_at)->format('d M y, H:i') ?: '-' }}</td>
                                    <td style="text-align: right;">
                                        @if(!$token->revoked_at)
                                            <form method="POST" action="{{ route('hr.external-requests.share.revoke', [$ticket, $token]) }}" onsubmit="return confirm('Revoke link ini?')">
                                                @csrf @method('PATCH')
        ...
        <div style="display: flex; flex-direction: column; gap: 20px;">
            ...
            <div class="pf-card-light">
                ...
                <div style="margin-top: 24px;">
                    ...
                    <div style="font-size: 20px; font-weight: 600; color: #111; letter-spacing: -0.02em;">
                        {{ optional($ticket->deadline)->format('d M Y') ?: '-' }}
                    </div>
                    @if(optional($ticket->deadline)->isPast() && $ticket->status_proses !== 'selesai')
                ...
                @if($ticket->tanggal_selesai)
                    <div style="margin-top: 24px; padding-top: 24px; border-top: 1px solid #f3f4f6;">
                        ...
                        <div style="font-size: 16px; font-weight: 600; color: #111;">
                            {{ $ticket->tanggal_selesai->format('d M Y') }}
                        </div>
                    </div>
                @endif
            </div>
            ...
            <div class="pf-card-dark">
                ...
                <form method="POST" action="{{ route('hr.external-requests.share.generate', $ticket) }}">
                    @csrf
