...
<body class="bg-[#E6F0F9] font-sans antialiased overflow-x-hidden min-h-screen selection:bg-[#4FA5F5] selection:text-white">
    ...
    <div class="mx-auto max-w-[1400px] px-3 py-6 sm:px-6 lg:py-10 relative z-10 pb-32 sm:pb-10">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6 sm:mb-8 bg-white/60 backdrop-blur-md p-4 sm:pr-6 rounded-[24px] sm:rounded-[30px] border border-white shadow-sm">
            <div class="flex items-center gap-4">
                <a href="{{ route('lab.dashboard') }}" class="flex h-12 w-12 sm:h-14 sm:w-14 flex-shrink-0 items-center justify-center rounded-full bg-white text-gray-500 shadow-sm transition-all hover:scale-105 hover:text-[#FF6B93]">
                ...
                <div>
                    ...
                    <p class="text-xs sm:text-sm font-medium text-gray-500 mt-0.5">Petugas: <span class="text-[#4FA5F5]">{{ auth()->user()->name ?? 'Administrator' }}</span></p>
                </div>
            </div>
        ...
        <div id="tab-satuan" class="space-y-6 transition-all duration-500 block">
            ...
            <div class="grid grid-cols-1 gap-4 sm:gap-5 sm:grid-cols-3">
                <div class="rounded-[20px] sm:rounded-[30px] border border-white bg-white/90 p-5 sm:p-6 shadow-[0_8px_20px_-6px_rgba(0,0,0,0.03)] backdrop-blur-xl flex flex-row sm:flex-col items-center sm:items-start justify-between">
                    ...
                    <p class="text-xl sm:text-2xl font-black text-gray-900 mt-0 sm:mt-2">{{ now()->translatedFormat('d M Y') }}</p>
                </div>
                <div class="rounded-[20px] sm:rounded-[30px] border border-white bg-white/90 p-5 sm:p-6 shadow-[0_8px_20px_-6px_rgba(0,0,0,0.03)] backdrop-blur-xl flex flex-row sm:flex-col items-center sm:items-start justify-between">
                    <p class="text-xs sm:text-sm font-extrabold text-gray-400 uppercase tracking-wider">Input Hari Ini</p>
                    <p class="text-xl sm:text-3xl font-black text-gray-900 mt-0 sm:mt-2">{{ number_format($totalInput) }} <span class="text-sm sm:text-lg text-gray-400 font-bold">Data</span></p>
                </div>
                <div class="rounded-[20px] sm:rounded-[30px] border border-white bg-white/90 p-5 sm:p-6 shadow-[0_8px_20px_-6px_rgba(0,0,0,0.03)] backdrop-blur-xl flex flex-row sm:flex-col items-center sm:items-start justify-between">
                    <p class="text-xs sm:text-sm font-extrabold text-gray-400 uppercase tracking-wider">Parameter Terisi</p>
                    <p class="text-xl sm:text-3xl font-black text-gray-900 mt-0 sm:mt-2">{{ number_format($uniqueParameter) }} <span class="text-sm sm:text-lg text-gray-400 font-bold">Jenis</span></p>
                </div>
            </div>

            {{-- KESATUAN QUEST GLOBAL & DAFTAR MISI --}}
                    ...
                    @else
                        ...
                        <div class="rounded-[20px] bg-[#E6F0F9] px-5 py-5 space-y-3 shadow-inner">
                            <div class="flex items-center justify-between">
                                ...
                                <span class="text-2xl font-black text-[#4FA5F5]">{{ number_format($questCompleted ?? 0) }}/{{ number_format($questTotal ?? 0) }}</span>
                            </div>
                            <div class="h-4 rounded-full bg-white overflow-hidden shadow-sm border border-[#4FA5F5]/20">
                                <div class="h-full rounded-full bg-gradient-to-r from-[#4FA5F5] to-[#10B981] transition-all duration-500 relative" style="width: {{ min(100, round((($questCompleted ?? 0) / $questTotal) * 100)) }}%">
                            ...
                            <p class="text-right text-xs font-bold text-gray-500">
                                {{ $questTotal > 0 ? round((($questCompleted ?? 0) / $questTotal) * 100) : 0 }}% selesai
                            </p>
                        </div>
                    @endif
                </div>

                {{-- LIST DAFTAR MISI PEMBENTUK GLOBAL --}}
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-5">
                    
                    {{-- SHIFT MISSIONS (Personal per 2, 4 jam) --}}
                    <div class="rounded-[24px] border border-blue-100 bg-gradient-to-br from-white to-blue-50/30 p-4 sm:p-5 shadow-sm hover:shadow-md transition-shadow">
                        ...
                        <div class="space-y-2.5 max-h-[360px] overflow-y-auto pr-1 no-scrollbar">
                            ...
                            @forelse($pendingMissions as $mission)
                                ...
                                @endphp
                                <div class="flex items-start gap-3 rounded-[18px] border border-blue-100 px-3 py-2.5 bg-slate-50 hover:bg-blue-50 transition-colors cursor-pointer" data-mission-shortcut data-parameter-name="{{ $mission->parameter_name }}" data-parameter-id="{{ $mission->parameter_id ?? '' }}">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        <p class="text-sm font-bold text-gray-800">{{ $mission->parameter_name }}</p>
                                        <div class="mt-1 flex flex-wrap items-center gap-2 text-[10px] font-semibold">
                                            <span class="rounded-full bg-white px-2 py-0.5 text-gray-500 border border-blue-100">{{ $mission->category }} • {{ $frequency }}</span>
                                            <span
                                                class="rounded-full px-2 py-0.5 bg-blue-100 text-blue-800"
                                                data-mission-reset-countdown
                                                data-reset-at="{{ $mission->mission_reset_iso }}"
                                                data-mission-done="0"
                                                data-mission-prefix="Tersedia"
                                            >
                                                Tersedia • reset {{ $resetLabel }}
                                            </span>
                                        </div>
                                    </div>
                                </div>
                            ...
                            @if($completedMissions->isNotEmpty())
                                ...
                                @foreach($completedMissions as $mission)
                                    ...
                                    <div class="flex items-start gap-3 rounded-[18px] border border-emerald-100 px-3 py-2.5 bg-emerald-50/60">
                                        ...
                                        <div class="min-w-0 flex-1">
                                            <p class="text-sm font-bold text-emerald-700 line-through decoration-emerald-500/70">{{ $mission->parameter_name }}</p>
                                            <div class="mt-1 flex flex-wrap items-center gap-2 text-[10px] font-semibold">
                                                <span class="rounded-full bg-white px-2 py-0.5 text-gray-500 border border-emerald-100">{{ $mission->category }} • {{ $frequency }}</span>
                                                <span class="rounded-full px-2 py-0.5 bg-emerald-100 text-emerald-700">
                                                    Selesai • reset {{ $resetLabel }}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div class="rounded-[24px] border-2 border-purple-200 bg-gradient-to-br from-white to-purple-50/30 p-4 sm:p-5 shadow-md">
                        ...
                        <div class="space-y-2.5 max-h-[360px] overflow-y-auto pr-1 no-scrollbar">
                            ...
                            {{-- PENDING DAILY MISSIONS --}}
                            @forelse($pendingDailyMissions as $mission)
                                @php
                                    $frequency = $mission->sampling_frequency ?? '-';
                                    $resetLabel = $mission->mission_reset_label ?? '-';
                                @endphp
                                <div class="flex items-start gap-3 rounded-[18px] border border-purple-100 px-3 py-2.5 bg-slate-50 hover:bg-purple-50 transition-colors cursor-pointer" data-mission-shortcut data-parameter-name="{{ $mission->parameter_name }}" data-parameter-id="{{ $mission->parameter_id ?? '' }}">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        <p class="text-sm font-bold text-gray-800">{{ $mission->parameter_name }}</p>
                                        <div class="mt-1 flex flex-wrap items-center gap-2 text-[10px] font-semibold">
                                            <span class="rounded-full bg-white px-2 py-0.5 text-gray-500 border border-purple-100">{{ $mission->category }} • {{ $frequency }}</span>
                                            <span class="rounded-full px-2 py-0.5 bg-purple-100 text-purple-800">
                                                Pending • reset {{ $resetLabel }}
                                            </span>
                                        </div>
                            ...
                            @if($completedDailyMissions->isNotEmpty())
                                <div class="mt-4 pt-3 border-t-2 border-emerald-200"></div>
                                <div class="text-xs font-bold text-emerald-600 uppercase tracking-wider px-1 mb-2">✓ Sudah Selesai</div>
                                @foreach($completedDailyMissions as $mission)
                                    @php
                                        $frequency = $mission->sampling_frequency ?? '-';
                                    @endphp
                                    <div class="flex items-start gap-3 rounded-[18px] border border-emerald-100 px-3 py-2.5 bg-emerald-50/60">
                ...
                <div id="form-container" class="fixed inset-x-0 bottom-0 z-50 translate-y-full transform transition-transform duration-300 ease-out lg:static lg:z-auto lg:translate-y-0 lg:col-span-4 lg:block">
                    ...
                    <form id="satuan-form" action="{{ route('lab.sampling.store') }}" method="POST" class="rounded-t-[32px] lg:rounded-[40px] border border-white bg-white/95 lg:bg-white/90 p-5 sm:p-8 shadow-[0_-10px_40px_rgba(0,0,0,0.1)] lg:shadow-[0_20px_50px_-12px_rgba(0,0,0,0.05)] backdrop-blur-2xl space-y-4 sm:space-y-5 lg:sticky lg:top-6 max-h-[85vh] overflow-y-auto no-scrollbar lg:max-h-none lg:overflow-visible">
                        ...
                        <div>
                            ...
                            <input type="text" name="source_unit" value="{{ old('source_unit', $defaultSourceUnit ?? 'PKS SSM') }}" placeholder="Contoh: PKS SSM" class="w-full rounded-[16px] sm:rounded-full border-none bg-[#F3F6F9] px-4 sm:px-5 py-3.5 text-sm font-bold text-gray-900 focus:outline-none focus:ring-4 focus:ring-[#4FA5F5]/20" required>
                        </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-1 gap-3 sm:gap-4">
                            <div>
                                ...
                                <select id="satuan-parameter-select" name="parameter_id" class="w-full rounded-[16px] sm:rounded-full border-none bg-[#F3F6F9] px-4 sm:px-5 py-3.5 text-sm font-bold text-gray-900 focus:outline-none focus:ring-4 focus:ring-[#4FA5F5]/20 cursor-pointer" required onchange="handleSatuanSelectChange()">
                                    ...
                                    @foreach($groupedParameters as $category => $items)
                                        <optgroup label="{{ $category }}" class="font-extrabold text-gray-500 bg-white">
                                            @foreach($items as $item)
                                                ...
                                                @endphp
                                                @if(!$isTot)
                                                    <option value="{{ $item->id }}" data-category-key="{{ $catLow }}" @selected((int) old('parameter_id') === (int) $item->id) class="text-gray-900 font-bold">
                                                        {{ $item->parameter_name }} ({{ $item->sampling_frequency ?: 'N/A' }})
                                                    </option>
                                                @endif
                                            @endforeach
                                        </optgroup>
                        ...
                        {{-- Hidden time input: auto-captured on form submit via JavaScript --}}
                            ...
                            <div class="rounded-[24px] bg-[#F7F9FC] p-4 border border-gray-50 space-y-3">
                                <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                    <div>
                                        ...
                                        <input type="text" inputmode="decimal" id="satuan_ffa_a" name="formula[a]" value="{{ old('formula.a') }}" class="w-full rounded-[14px] border-none bg-white px-4 py-3 text-sm font-bold text-gray-900 shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)] focus:outline-none focus:ring-4 focus:ring-[#4FA5F5]/20" placeholder="0.00" disabled>
                                    </div>
                                    <div>
                                        <label class="mb-1 block text-[10px] sm:text-xs font-extrabold text-gray-500 uppercase tracking-wider">Vol Titrasi (B)</label>
                                        <input type="text" inputmode="decimal" id="satuan_ffa_b" name="formula[b]" value="{{ old('formula.b') }}" class="w-full rounded-[14px] border-none bg-white px-4 py-3 text-sm font-bold text-gray-900 shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)] focus:outline-none focus:ring-4 focus:ring-[#4FA5F5]/20" placeholder="0.00" disabled>
                                    </div>
                                    <div>
                                        <label class="mb-1 block text-[10px] sm:text-xs font-extrabold text-gray-500 uppercase tracking-wider">Normalitas (C)</label>
                                        <input type="text" inputmode="decimal" id="satuan_ffa_c" name="formula[c]" value="{{ old('formula.c') }}" class="w-full rounded-[14px] border-none bg-white px-4 py-3 text-sm font-bold text-gray-900 shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)] focus:outline-none focus:ring-4 focus:ring-[#4FA5F5]/20" placeholder="0.00" disabled>
                                    </div>
                                </div>
                            ...
                            <div class="rounded-[24px] bg-[#F7F9FC] p-4 border border-gray-50 space-y-3">
                                <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                                    <div>
                                        ...
                                        <input type="text" inputmode="decimal" id="satuan_dirt_l" name="formula[l]" value="{{ old('formula.l') }}" class="w-full rounded-[14px] border-none bg-white px-4 py-3 text-sm font-bold text-gray-900 shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)] focus:outline-none focus:ring-4 focus:ring-[#4FA5F5]/20" placeholder="0.00" disabled>
                                    </div>
                                    <div>
                                        <label class="mb-1 block text-[10px] sm:text-xs font-extrabold text-gray-500 uppercase tracking-wider">W+Sampel (M)</label>
                                        <input type="text" inputmode="decimal" id="satuan_dirt_m" name="formula[m]" value="{{ old('formula.m') }}" class="w-full rounded-[14px] border-none bg-white px-4 py-3 text-sm font-bold text-gray-900 shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)] focus:outline-none focus:ring-4 focus:ring-[#4FA5F5]/20" placeholder="0.00" disabled>
                                    </div>
                                    <div>
                                        <label class="mb-1 block text-[10px] sm:text-xs font-extrabold text-gray-500 uppercase tracking-wider">Cr. Kosong (O)</label>
                                        <input type="text" inputmode="decimal" id="satuan_dirt_o" name="formula[o]" value="{{ old('formula.o') }}" class="w-full rounded-[14px] border-none bg-white px-4 py-3 text-sm font-bold text-gray-900 shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)] focus:outline-none focus:ring-4 focus:ring-[#4FA5F5]/20" placeholder="0.00" disabled>
                                    </div>
                                    <div>
                                        <label class="mb-1 block text-[10px] sm:text-xs font-extrabold text-gray-500 uppercase tracking-wider">Cr. + Dirt (P)</label>
                                        <input type="text" inputmode="decimal" id="satuan_dirt_p" name="formula[p]" value="{{ old('formula.p') }}" class="w-full rounded-[14px] border-none bg-white px-4 py-3 text-sm font-bold text-gray-900 shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)] focus:outline-none focus:ring-4 focus:ring-[#4FA5F5]/20" placeholder="0.00" disabled>
                                    </div>
                                </div>
                                <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                    <div class="rounded-[16px] bg-white px-4 py-3 border border-gray-100">
                        ...
                        <div>
                            ...
                            <textarea name="notes" rows="2" placeholder="Keterangan tambahan..." class="w-full rounded-[16px] sm:rounded-[24px] border-none bg-[#F3F6F9] px-4 sm:px-5 py-3 sm:py-3.5 text-sm font-bold text-gray-900 focus:outline-none focus:ring-4 focus:ring-[#4FA5F5]/20 resize-none">{{ old('notes') }}</textarea>
                        </div>
                ...
                <div class="lg:col-span-8">
                    ...
                    @else
                        <div class="space-y-8 sm:space-y-10">
                            @foreach($groupedHistory as $shift => $categories)
                                <div>
                                    <div class="flex items-center gap-3 mb-4 pl-1">
                                        <span class="flex h-8 w-8 items-center justify-center rounded-full bg-[#4FA5F5] text-white font-black shadow-md">{{ $shift }}</span>
                                        <h3 class="text-xl font-black text-gray-900 tracking-tight">Shift {{ $shift }}</h3>
                                    </div>
                                        ...
                                        @foreach($categories as $category => $entries)
                                            <div class="rounded-[20px] sm:rounded-[24px] border border-white bg-white/95 p-4 sm:p-5 shadow-[0_10px_30px_-10px_rgba(0,0,0,0.05)] hover:shadow-[0_15px_35px_-10px_rgba(0,0,0,0.08)] transition-all">
                                                ...
                                                <div class="flex items-center gap-2 mb-4 border-b border-gray-100 pb-3">
                                                    ...
                                                    <h4 class="text-xs sm:text-sm font-extrabold text-[#FF6B93] uppercase tracking-wider">{{ $category }}</h4>
                                                </div>
                                                        ...
                                                        <div class="flex items-center justify-between group">
                                                            <div class="flex items-center gap-3 overflow-hidden pr-2">
                                                                <span class="text-[10px] sm:text-xs font-bold text-gray-400 w-8 sm:w-10 shrink-0">
                                                                    {{ \Carbon\Carbon::parse($entry->measured_at ?? now())->format('H:i') }}
                                                                </span>
                                                            ...
                                                            <div class="flex items-end gap-1.5 shrink-0 bg-[#F3F6F9] px-2 py-1 rounded-lg">
                                                                <span class="text-sm sm:text-base font-black text-gray-900">
                                                                    {{ $entry->measured_text ?: $entry->measured_value ?: '-' }}
                                                                </span>
        ...
        <div id="tab-batch" class="hidden transition-all duration-500">
            <div class="max-w-3xl mx-auto">
                <form action="{{ route('lab.sampling.store') }}" method="POST" class="space-y-6 sm:space-y-8">
                    @csrf
                    ...
                    <div class="rounded-[24px] sm:rounded-[40px] border border-white bg-white/90 p-5 sm:p-8 shadow-[0_20px_50px_-12px_rgba(0,0,0,0.05)] backdrop-blur-xl space-y-4 sm:space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="source_unit" value="{{ old('source_unit', $defaultSourceUnit ?? 'PKS SSM') }}" placeholder="Contoh: PKS SSM" class="w-full rounded-[16px] sm:rounded-full border-none bg-[#F3F6F9] px-4 sm:px-6 py-3 sm:py-4 text-sm sm:text-base font-bold text-gray-900 transition-shadow focus:outline-none focus:ring-4 focus:ring-[#4FA5F5]/20 focus:bg-white" required>
                        </div>
                    </div>

                    @forelse($groupedParameters ?? [] as $category => $items)
                        <div class="space-y-3 sm:space-y-4">
                            <div class="flex items-center gap-2 sm:gap-3 px-1 sm:px-2">
                                <div class="h-6 sm:h-8 w-2 rounded-full bg-[#4FA5F5]"></div>
                                <h2 class="text-lg sm:text-xl font-extrabold text-gray-900 tracking-tight">{{ $category }}</h2>
                            </div>
                            ...
                            <div class="rounded-[24px] sm:rounded-[40px] border border-white bg-white/90 p-3 sm:p-6 shadow-[0_20px_50px_-12px_rgba(0,0,0,0.05)] backdrop-blur-xl space-y-3 sm:space-y-4">
                                @foreach($items as $item)
                                    ...
                                    <div
                                        class="rounded-[20px] sm:rounded-[24px] border {{ $isTotal ? 'border-[#4FA5F5] bg-[#E6F0F9]/50' : 'border-gray-50 bg-[#F7F9FC]' }} p-4 sm:p-5 transition-all hover:bg-white hover:shadow-md hover:ring-2 hover:ring-[#4FA5F5]/20 group"
                                        data-parameter-card
                                        data-category="{{ $category }}"
                                        data-category-key="{{ $categoryLower }}"
                                        data-parameter-id="{{ $item->id }}"
                                        data-parameter-name="{{ $item->parameter_name }}"
                                        data-is-total="{{ $isTotal ? '1' : '0' }}"
                                    >
                                        <div class="mb-3 sm:mb-4 flex flex-col sm:flex-row sm:items-start justify-between gap-2 sm:gap-3">
                                            <div>
                                                <p class="text-base sm:text-lg font-bold {{ $isTotal ? 'text-[#4FA5F5]' : 'text-gray-900' }} group-hover:text-[#4FA5F5] transition-colors leading-tight">
                                                    {{ $item->parameter_name }}
                                                    @if($isTotal)
                                                ...
                                                <div class="mt-1.5 flex flex-wrap items-center gap-1.5 sm:gap-2 text-[10px] sm:text-xs font-medium text-gray-400">
                                                    <span class="bg-white px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-md shadow-sm border border-gray-100">STD: <span class="text-gray-700 font-bold">{{ $item->standard_text ?? '-' }}</span></span>
                                                    <span class="bg-white px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-md shadow-sm border border-gray-100">Freq: <span class="text-gray-700 font-bold">{{ $item->sampling_frequency ?? '-' }}</span></span>
                                                </div>
                                            </div>
                                        ...
                                        @if($isFfa)
                                            <div class="space-y-3">
                                                <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                                    <div>
                                                        <label class="mb-1 block text-[10px] sm:text-xs font-extrabold text-gray-500 uppercase tracking-wider">Berat Sample (A)</label>
                                                        <input type="text" inputmode="decimal" data-ffa-a name="formula[{{ $item->id }}][a]" value="{{ old('formula.' . $item->id . '.a') }}" class="w-full rounded-[14px] border-none bg-white px-4 py-3 text-sm font-bold text-gray-900 shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)] focus:outline-none focus:ring-4 focus:ring-[#4FA5F5]/20" placeholder="0.00">
                                                    </div>
                                                    <div>
                                                        <label class="mb-1 block text-[10px] sm:text-xs font-extrabold text-gray-500 uppercase tracking-wider">Volume Titrasi (B)</label>
                                                        <input type="text" inputmode="decimal" data-ffa-b name="formula[{{ $item->id }}][b]" value="{{ old('formula.' . $item->id . '.b') }}" class="w-full rounded-[14px] border-none bg-white px-4 py-3 text-sm font-bold text-gray-900 shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)] focus:outline-none focus:ring-4 focus:ring-[#4FA5F5]/20" placeholder="0.00">
                                                    </div>
                                                    <div>
                                                        ...
                                                        <input type="text" inputmode="decimal" data-ffa-c name="formula[{{ $item->id }}][c]" value="{{ old('formula.' . $item->id . '.c') }}" class="w-full rounded-[14px] border-none bg-white px-4 py-3 text-sm font-bold text-gray-900 shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)] focus:outline-none focus:ring-4 focus:ring-[#4FA5F5]/20" placeholder="0.00">
                                                    </div>
                                                </div>
                                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                                    <div class="rounded-[16px] bg-white px-4 py-3 border border-gray-100">
                                                    ...
                                                    <div class="rounded-[16px] bg-white px-4 py-3 border border-[#4FA5F5]/20">
                                                        <p class="text-[10px] font-extrabold uppercase tracking-wider text-[#4FA5F5]">Final FFA</p>
                                                        <input type="text" inputmode="decimal" readonly name="measurements[{{ $item->id }}]" value="{{ old('measurements.' . $item->id) }}" data-formula-result data-formula-type="ffa" class="mt-1 w-full border-none bg-transparent p-0 text-2xl font-black text-gray-900 focus:outline-none">
                                                    </div>
                                                </div>
                                                <p class="text-[10px] sm:text-xs font-medium text-gray-500">Formula: (B × C × 25.6) / A</p>
                                            </div>
                                            ...
                                            <div class="space-y-3">
                                                <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                                                    <div>
                                                        ...
                                                        <input type="text" inputmode="decimal" data-dirt-l name="formula[{{ $item->id }}][l]" value="{{ old('formula.' . $item->id . '.l') }}" class="w-full rounded-[14px] border-none bg-white px-4 py-3 text-sm font-bold text-gray-900 shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)] focus:outline-none focus:ring-4 focus:ring-[#4FA5F5]/20" placeholder="0.00">
                                                    </div>
                                                    <div>
                                                        <label class="mb-1 block text-[10px] sm:text-xs font-extrabold text-gray-500 uppercase tracking-wider">W+Sample (M)</label>
                                                        <input type="text" inputmode="decimal" data-dirt-m name="formula[{{ $item->id }}][m]" value="{{ old('formula.' . $item->id . '.m') }}" class="w-full rounded-[14px] border-none bg-white px-4 py-3 text-sm font-bold text-gray-900 shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)] focus:outline-none focus:ring-4 focus:ring-[#4FA5F5]/20" placeholder="0.00">
                                                    </div>
                                                    <div>
                                                        ...
                                                        <input type="text" inputmode="decimal" data-dirt-o name="formula[{{ $item->id }}][o]" value="{{ old('formula.' . $item->id . '.o') }}" class="w-full rounded-[14px] border-none bg-white px-4 py-3 text-sm font-bold text-gray-900 shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)] focus:outline-none focus:ring-4 focus:ring-[#4FA5F5]/20" placeholder="0.00">
                                                    </div>
                                                    <div>
                                                        <label class="mb-1 block text-[10px] sm:text-xs font-extrabold text-gray-500 uppercase tracking-wider">Crucible+Dirt (P)</label>
                                                        <input type="text" inputmode="decimal" data-dirt-p name="formula[{{ $item->id }}][p]" value="{{ old('formula.' . $item->id . '.p') }}" class="w-full rounded-[14px] border-none bg-white px-4 py-3 text-sm font-bold text-gray-900 shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)] focus:outline-none focus:ring-4 focus:ring-[#4FA5F5]/20" placeholder="0.00">
                                                    </div>
                                                </div>
                                                <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                                    <div class="rounded-[16px] bg-white px-4 py-3 border border-gray-100">
                                                    ...
                                                    <div class="rounded-[16px] bg-white px-4 py-3 border border-[#4FA5F5]/20">
                                                        <p class="text-[10px] font-extrabold uppercase tracking-wider text-[#4FA5F5]">Final % Dirt</p>
                                                        <input type="text" inputmode="decimal" readonly name="measurements[{{ $item->id }}]" value="{{ old('measurements.' . $item->id) }}" data-formula-result data-formula-type="dirt" class="mt-1 w-full border-none bg-transparent p-0 text-2xl font-black text-gray-900 focus:outline-none">
                                                    </div>
                                                </div>
                                                <p class="text-[10px] sm:text-xs font-medium text-gray-500">Formula: ((P - O) / (M - L)) × 100</p>
                                            </div>
                                        @elseif($isTotal)
                                            <div class="space-y-2">
                                                <div class="rounded-[16px] bg-white px-4 py-3 border border-[#FCE57F]/40">
                                                    ...
                                                    <input type="text" readonly inputmode="decimal" name="measurements[{{ $item->id }}]" value="{{ old('measurements.' . $item->id) }}" data-total-result data-total-category="{{ $categoryLower }}" class="mt-1 w-full border-none bg-transparent p-0 text-2xl font-black text-gray-900 focus:outline-none">
                                                </div>
                                                <p class="text-[10px] sm:text-xs font-medium text-gray-500">Total dihitung otomatis dari parameter lain pada kategori ini.</p>
                                            </div>
                                        @else
                                            <div class="relative">
                                                <input type="text" inputmode="decimal" name="measurements[{{ $item->id }}]" value="{{ $directValue }}" placeholder="Hasil ukur..." data-direct-result data-category-key="{{ $categoryLower }}" class="w-full rounded-[16px] sm:rounded-2xl border-none bg-white px-4 sm:px-5 py-3 sm:py-4 text-sm sm:text-base font-bold text-gray-900 shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)] transition-shadow placeholder:text-gray-300 focus:outline-none focus:ring-4 focus:ring-[#4FA5F5]/30 focus:bg-white">
                                            </div>
                                        @endif
                                    </div>
                                @endforeach
    ...
    {{-- HIDDEN STATE TRACKING FOR END SHIFT --}}
    <input type="hidden" id="shift-end-state" value="active" data-shift="{{ $activeShift }}">
    <input type="hidden" id="shift-mvp-status" value="0">
    <input type="hidden" id="daily-missions-completed" value="{{ collect($missionGroups ?? [])->get('Target Harian', collect())->filter(fn($m) => $m->mission_completed ?? false)->count() }}">
    <input type="hidden" id="daily-missions-total" value="{{ collect($missionGroups ?? [])->get('Target Harian', collect())->count() }}">

    {{-- END SHIFT WARNING MODAL --}}
        ...
        {{-- SHIFT SUMMARY REPORT CARD (Replaces form when shift ends) --}}
        <div id="shiftSummaryReport" class="fixed inset-0 z-40 opacity-0 pointer-events-none transition-opacity duration-500 bg-gradient-to-br from-slate-900/50 via-slate-900/40 to-slate-900/50 backdrop-blur-lg">
            <div class="flex items-center justify-center min-h-screen p-4">
                <div class="rounded-[40px] border border-white bg-white/95 p-8 sm:p-12 shadow-2xl max-w-2xl w-full backdrop-blur-xl transform transition-all duration-500 scale-95">
                    ...
                    {{-- MVP BADGE SECTION (Only if user completed daily mission) --}}
    ...
    <script>
        ...
        function exitShiftReport() {
            window.location.href = "{{ route('lab.dashboard') }}";
        }
    </script>
</body>
</html>